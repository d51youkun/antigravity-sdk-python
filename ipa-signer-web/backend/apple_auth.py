...
class AppleSigner:
    ...
    def create_p12_bytes(
        self,
        private_key_pem: bytes,
        cert_der: bytes,
        password: str = "",
    ) -> bytes:
        """Create PKCS#12 bytes from private key and certificate."""
        from cryptography import x509
        from cryptography.hazmat.primitives import serialization
        from cryptography.hazmat.primitives.serialization import (
...
class WebSigningSession:
    """In-memory Apple ID session for the web API."""

    def __init__(self, anisette_url: str = DEFAULT_ANISETTE_URL):
        self.signer = AppleSigner(anisette_url)
        self.apple_id: Optional[str] = None
        self._password: Optional[str] = None
        self.tfa_type: Optional[str] = None
    ...
    def start_login(self, apple_id: str, password: str) -> dict:
        ...
        result = self.signer.auth.authenticate(apple_id, password)
        status = result["status"]

        if self.signer.auth.needs_2fa(status):
            self.tfa_type = self.signer.auth.get_2fa_type(status)
            if self.tfa_type == "trusted_device":
                self.signer.auth.send_2fa_trusted_device()
            else:
                self.signer.auth.send_2fa_sms()
            return {
                "needs_2fa": True,
                "tfa_type": self.tfa_type,
            }

        return self._finalize_login()

    def submit_2fa(self, code: str) -> dict:
        if not self.tfa_type:
            raise RuntimeError("Two-factor authentication was not started")

        if self.tfa_type == "trusted_device":
            ok = self.signer.auth.submit_2fa_code(code)
        else:
            ok = self.signer.auth.submit_2fa_sms_code(code)
    ...
    def _finalize_login(self) -> dict:
        if not self.apple_id or not self._password:
            raise RuntimeError("Missing Apple ID credentials")

        self.signer.auth.authenticate(self.apple_id, self._password)
        self.signer.auth.get_xcode_token()
        self.signer.portal = DeveloperPortal(self.signer.auth)
        self.teams = self.signer.portal.list_teams()
        self.logged_in = True
        self.tfa_type = None
        ...
        return {
            "needs_2fa": False,
            "teams": [
                {"team_id": t["teamId"], "name": t.get("name", t["teamId"])}
                for t in self.teams
            ],
        }
    ...
    def provision(
        ...
        if not self.logged_in or not self.signer.portal:
            raise RuntimeError("Not logged in")

        if team_id:
            self.signer.portal.select_team(team_id)
        else:
            self.signer.portal.select_team()

        private_key_pem, profile_bytes, cert_der, actual_bundle_id = (
            self.signer.provision(udid, bundle_id, app_name)
        )
        p12_bytes = self.signer.create_p12_bytes(
            private_key_pem, cert_der, password=p12_password
        )

        return {
            "bundle_id": actual_bundle_id,
            "team_id": self.signer.portal.team_id,
            "p12_base64": base64.b64encode(p12_bytes).decode("ascii"),
